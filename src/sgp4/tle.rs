...
impl TLE {
    ...
    pub fn parse_lines(&mut self, line1: &String, line2: &String) {
        ...
        let tokens: Vec<&str> = line2.split_whitespace().collect();
        if tokens.len() >= 11 {
            self.start_min = tokens[tokens.len() - 3].parse::<f64>().unwrap_or(0.0);
            self.stop_min = tokens[tokens.len() - 2].parse::<f64>().unwrap_or(0.0);
            self.step_min = tokens[tokens.len() - 1].parse::<f64>().unwrap_or(0.0);
        } else {
            self.start_min = 0.0;
            self.stop_min = 0.0;
            self.step_min = 0.0;
...
pub fn read_tles<P>(tlefile: P) -> io::Result<Vec<TLE>>
where
    P: AsRef<Path>,
{
    ...
    for line in read_lines(tlefile)? {
        let current = line?;
        ...
        match trimmed.chars().next().unwrap() {
            ...
            '2' => {
                if let Some(prev_line) = line1.take() {
                    let mut tle = TLE::new();
                    tle.name = pending_name.clone();
                    tle.parse_lines(&prev_line, &trimmed.to_string());
                    tles.push(tle);
                    pending_name.clear();
                }
            }
            _ => {
                pending_name = trimmed.to_string();
            }
        }
    }
