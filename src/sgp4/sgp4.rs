...
pub fn dpper(
    ...
    let (mut f2, mut f3, mut pe, mut pgh, mut ph, mut pinc, mut pl): (
        f64,
        f64,
        f64,
        f64,
...
pub fn dscom(
    ...
    /* --------------------- local variables ------------------------ */
    //let mut lsflg:i32;
    let (mut a1, mut a2, mut a3, mut a4, mut a5, mut a6, mut a7): (
        f64,
        f64,
        f64,
        f64,
    ...
    let (mut x1, mut x2, mut x3, mut x4, mut x5, mut x6, mut x7): (
        f64,
        f64,
        f64,
        f64,
    ...
    let (mut x8, xnodce, xnoi, mut zcosg, zcosgl, mut zcosh, zcoshl): (
        f64,
        f64,
        f64,
        f64,
    ...
    let (mut zcosi, zcosil, mut zsing, zsingl, mut zsinh, zsinhl, mut zsini): (
        f64,
        f64,
        f64,
        f64,
...
pub fn dsinit(tc: f64, xpidot: f64, rec: &mut ElsetRec) {
    // --------------------- local variables ------------------------ */
    ...
    if rec.irez != 0 {
        ...
        if rec.irez == 1 {
            ...
            f311 =
                0.9375 * rec.sinim * rec.sinim * (1.0 + 3.0 * rec.cosim) - 0.75 * (1.0 + rec.cosim);
            f330 = 1.0 + rec.cosim;
...
pub fn dspace(tc: f64, rec: &mut ElsetRec) {
    let mut iretn: i32;
...
pub fn sgp4init(opsmode: char, satrec: &mut ElsetRec) {
    // --------------------- local variables ------------------------ */
    let cc1sq: f64;
    ...
    if (satrec.omeosq >= 0.0) || (satrec.no_unkozai >= 0.0) {
        ...
        satrec.cc5 =
            2.0 * coef1 * satrec.ao * satrec.omeosq * (1.0 + 2.75 * (etasq + eeta) + eeta * etasq);
        ...
        if fabs(satrec.cosio + 1.0) > 1.5e-12 {
            satrec.xlcof = -0.25 * satrec.j3oj2 * satrec.sinio * (3.0 + 5.0 * satrec.cosio)
                / (1.0 + satrec.cosio);
        } else {
        ...
        if satrec.isimp != 1 {
            ...
            satrec.d4 =
                0.5 * temp * satrec.ao * tsi * (221.0 * satrec.ao + 31.0 * sfour) * satrec.cc1;
...
pub fn sgp4(satrec: &mut ElsetRec, tsince: f64, r: &mut [f64], v: &mut [f64]) -> bool {
    ...
    let (cos2u, mut coseo1, cosi, mut cosip, cosisq, cossu, cosu): (
        f64,
        f64,
        f64,
        f64,
    ...
    let (temp1, temp2, mut tempa, mut tempe, mut templ, u, ux): (
        f64,
        f64,
        f64,
        f64,
    ...
    if pl < 0.0 {
        ...
        rvdot =
            rvdotl + satrec.nm * temp1 * (satrec.x1mth2 * cos2u + 1.5 * satrec.con41) / satrec.xke;
...
pub fn atan2(num: f64, num2: f64) -> f64 {
    return num.atan2(num2);
}
...
pub fn pow(num: f64, num2: f64) -> f64 {
    return num.powf(num2);
}

pub fn floor(num: f64) -> f64 {
    return num.floor();
}
pub fn getgravconst(whichconst: i32, rec: &mut ElsetRec) {
    ...
    match whichconst {
        ...
        WGS72 => {
            ...
            rec.radiusearthkm = 6378.135; // km
            rec.xke =
                60.0 / sqrt(rec.radiusearthkm * rec.radiusearthkm * rec.radiusearthkm / rec.mu);
            rec.tumin = 1.0 / rec.xke;
            rec.j2 = 0.001082616;
            rec.j3 = -0.00000253881;
            rec.j4 = -0.00000165597;
...
pub fn fmod(numer: f64, denom: f64) -> f64 {
    let tquot = floor(numer / denom) as i64;
    return numer - (tquot as f64) * denom;
}
...
pub fn jday(year: i32, mon: i32, day: i32, hr: i32, minute: i32, sec: f64) -> (f64, f64) {
    ...
    jdfrac = (sec as f64 + (minute as f64) * 60.0 + (hr as f64) * 3600.0) / 86400.0;

    // check that the day and fractional day are correct
    if jdfrac.abs() > 1.0 {
        let dtt = floor(jdfrac);
        jd = jd + dtt;
        jdfrac = jdfrac - dtt;
    }
